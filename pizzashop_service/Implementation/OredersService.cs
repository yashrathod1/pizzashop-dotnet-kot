...
public class OredersService : IOredersService
{
    ...
    public async Task<OrderDetailsViewModel?> GetOrderDetailsAsync(int orderId)
    {
        ...
        var paidOn = order.Payments?.FirstOrDefault(p => p.PaymentStatus)?.Createdat ?? DateTime.MinValue;
        ...
        var orderTax = order.OrderTaxesMappings
        ...
        var orderTable = order.OrdersTableMappings.Select(ott => new OrdersTableMapViewModel
        {
    ...
    {
        // Fetch order details
        var model = await GetOrderDetailsAsync(orderId);

        if (model == null)
            throw new Exception("Order not found!");

        // Render CSHTML View as HTML string
        string htmlContent = await _razorRenderer.RenderViewToStringAsync("Orders/OrderInvoice", model);

        // Create the Select.Pdf HTML to PDF converter
        var converter = new HtmlToPdf();

        // Set converter options if necessary
