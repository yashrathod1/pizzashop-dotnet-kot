...
public class TableAndSectionController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var permission = await PermissionHelper.GetPermissionsAsync(HttpContext, "TableAndSection");
    ...
    public async Task<IActionResult> UpdateSection([FromBody] SectionsViewModal model)
    {
        ...
        var DuplicateSection = await _tableAndSectionService.GetSectionByNameAsync(model.Name);
        ...
        var existingSection = await _tableAndSectionService.GetSectionById(model.Id);
    ...
    public async Task<IActionResult> EditTable([FromBody] TableViewModel model)
    {
        ...
        var DuplicateSection = await _tableAndSectionService.GetTableByNameAsync(model.Name);
        if (DuplicateSection != null && DuplicateSection.Id != model.Id)
        {
            return Json(new { success = false, message = "A Section with this name already exists." });
        }

        var existingTable = await _tableAndSectionService.GetTableById(model.Id);
        if (existingTable == null)
        {
